...
void SEC_storage_api_get_invoice_id(unsigned char *pucBuff, unsigned char *pucInvoiceID) {
	...
	while(1) {
		...
		pucBuff += sizeof(struct SEC_TLV);  // 更新缓冲区偏移的位置
 ...
 */
static
int SEC_storage_api_send_by_fd(unsigned char *pucBuff, int iDataLen) {
	...
	while(iLen < iDataLen) { // 判断是否全部数据都发完毕
		pstHeader = (struct SEC_Header*)(pucBuff + iLen);  // 解析TLV头部
		if(pstHeader->ucIsLast == 1) {
			// 获取该请求的状态信息，判断是否所有节点发送数据完毕
			struct SEC_RecvStatus *pstRecvStatus = (struct SEC_RecvStatus*)(pstHeader->pointer2);
		...
		// 发送数据
		if(SEC_storage_api_sendEx(pstHeader->iFd, pucBuff + iLen, pstHeader->uiLen + sizeof(struct SEC_Header)) == FAILURE) {
			printf(__FILE__, __LINE__, __FUNCTION__, "In up thread, send to client error, ignore it.\n");
		}
		iLen = iLen + pstHeader->uiLen + sizeof(struct SEC_Header);  // 更新iLen
	}
	return SUCCESS;
}
...
int SEC_storage_api_send_data(unsigned char *pucBuff, int iWlen) {
	...
	while(iSendLen < iWlen) {
		...
		if(strlen(aucInvoiceID) == 0) { // 如果请求中不包含发票ID
			...
			pstHeader->pointer2 = (void*)pstRecvStatus;
			...
			switch(pstHeader->ucCmd) {
				...
				case CMD_STORE_OFD :
					...
					// 进行哈希计算和取模计算
					uiHash = SEC_storage_api_sdbmHash(aucInvoiceID);
				...
				case CMD_DOWNLOAD_OFD :
					...
					for(i = 0; i < g_pstHandle->pstGroup[uiIndex].iDeviceNum; i++) {
						if(SEC_storage_api_sendEx(g_pstHandle->pstGroup[uiIndex].piSockFd[i], pucBuff + iSendLen, uiDataLen + sizeof(struct SEC_Header)) == FAILURE) {
							printf("In epoll thread, lost connection with sec_gate %s, please check.\n", g_pstHandle->pstGroup[uiIndex].pstAddr[i]);
						}
					}
				break;
				default :
...
/**
 * 函数功能：检查完整作业包的长度，并修改包头的socket fd标识
 * 参数：
 *     @pucBuff[in]：存放数据的缓冲区
 *     @iDataLen[in]：数据长度
 *     @iSockFd[in]：socket文件描述符
 * 返回值：有效作业包长度
 */
static 
int SEC_storage_api_check_and_modify(unsigned char *pucBuff, int iDataLen, int iSockFd) {

	int iFullLen;					// 记录总的有效包长度
	int iLeft;						// 剩余数据长度
	struct SEC_Header *pstHeader;	// 数据包头
	unsigned int uiLen;				// 解析每个数据包的长度

	// 初始化变量
	iFullLen = 0;
	iLeft = iDataLen;
	
	while(iLeft >= sizeof(struct SEC_Header)) { // 如果数据够一个包头

		pstHeader = (struct SEC_Header*)(pucBuff + iFullLen);
		
		uiLen = pstHeader->uiLen;
		if(iLeft < uiLen + sizeof(struct SEC_Header)) {	// 判断剩余的数据够不够一个完整的作业包
			break;
		}
		// 修改socket fd标识
		pstHeader->iFd = iSockFd;
 ...
 *           不同线程的监听事件中。但可能会出现一个线程存活率很高。另一线程客户端大部分都断开连接导致不均衡。
...
void *SEC_storage_api_down_thread(void *param) {
	...
	struct SEC_FirstGateway *pstGateway;	// 网关状态信息 
			...
			case S100 :
				...
				for(i = 0; i < iFds; i++) {
					...
					if(pstEvents[i].events & EPOLLIN) {
						...
						if(iLen > 0) {
							//返回有效作业包的总长度
							iFullLen = SEC_storage_api_check_and_modify(pucWbuff + iWlen, iLen + pstEd->iLen, pstEd->iFd);
...
int SEC_storage_api_check_pkt(unsigned char *pucBuff, int iDataLen) {
	...
	while(iLeft >= sizeof(struct SEC_Header)) { // 如果剩余的数据够一个包头
		...
		pstHeader = (struct SEC_Header*)(pucBuff + iFullLen);  // 解析包头 
...
/**
 * 函数功能：为events分配空间，该数据结构线程独立，作为线程局部变量即可。
 *        1）将程序初始化时已经和db_serve建立好的连接添加到Epoll当中，等待事件发生，期间不会再有新连接到来
 *        2）如果连接有输入则读入数据并处理。先将该连接上次未处理完的数据加入buff，
 *           再将从该连接读入的数据加入buff，然后进行处理。将本次未处理完的数据再保存到每个连接的data中，
 *           下次事件发生时处理。
 *        3）将断开的连接释放掉。
 * 参数：
...
void *SEC_storage_api_up_thread(void *param) {
	...
	for(;;) {
		...
		for(i = 0; i < iFds; i++) {
			...
			// 处理数据的业务逻辑部分
			if(pstEvents[i].events & EPOLLIN) {
				//printf("in up thread read events[i].events = %x, ed->fd = %d\n", pstEvents[i].events, pstEd->iFd);
				//SEC_storage_api_log_print("In up thread read from sec_gate %s\n", SEC_storage_api_get_ip_by_fd(g_pstHandle, pstEd->iFd));
		...
		// 将所有数据返回给对应的用户
		SEC_storage_api_send_by_fd(pucWbuff, iWlen);
		iWlen = 0;
	}
	return;
}
 ...
 */
static 
int SEC_storage_api_first_gateway_init(struct SEC_FirstGateway *pstGateway, int iThreadNum, int iMaxConn) {
	...
	// 对Epoll进行同步操作的互斥量数组，每个Epoll句柄拥有一个
 ...
 */
static
int SEC_storage_api_first_gateway_start(struct SEC_FirstGateway *pstGateway, unsigned short usPort) {
	...
	if(pthread_create(&pstGateway->upThread, NULL, SEC_storage_api_up_thread, (void*)&pstEp[0]) != 0) {
		SEC_storage_api_err_print(__FILE__, __LINE__, __FUNCTION__, "pthread_thread()");
		return FAILURE;
	}
...
int main() {
	...
	struct SEC_FirstGateway stGateway;
	memset(&stGateway, 0, sizeof(struct SEC_FirstGateway));
	...
	// 传入Epoll线程数和总的连接数
	if(SEC_storage_api_first_gateway_init(&stGateway, THREAD_NUM, EVENTS_NUM) == FAILURE) {
	...
	// 启动网关
	if(SEC_storage_api_first_gateway_start(&stGateway, PORT) == FAILURE) {
